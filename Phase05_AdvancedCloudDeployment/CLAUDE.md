...
- **Cloud Platform**: Oracle Cloud Infrastructure (OKE - Oracle Kubernetes Engine) - Always Free tier
- **Orchestration**: Kubernetes with Dapr sidecars and Kafka operators
- **Pub/Sub**: Dapr with Kafka as the underlying transport for event-driven architecture
- **State Management**: Dapr state stores with PostgreSQL backend
- **Bindings**: Dapr cron bindings for scheduled tasks and reminders
- **Secrets**: Dapr secret stores with Kubernetes secrets backend
...
- **State Management**: Conversation state storage using Dapr state stores
...
- **Backend Service**: FastAPI application (single entry point) with MCP tools and Dapr sidecar
- **Kafka Cluster**: Self-hosted with Strimzi operator or managed with Redpanda Cloud
- **Dapr Sidecars**: Running alongside each service for pub/sub, state, and service invocation
...
1. **Prerequisites Setup**:
   - Sign up for Oracle Cloud (Always Free tier)
   - Install OCI CLI and configure credentials
   - Install kubectl and configure for OKE access
   - Install Dapr CLI and initialize on OKE cluster

2. **Cloud Infrastructure**:
   - Create OKE cluster with 4 OCPUs and 24GB RAM (Always Free tier)
   - Configure kubectl to connect to OKE cluster
   - Install Dapr on OKE cluster with full capabilities

3. **Event Infrastructure**:
   - Deploy Kafka cluster using Strimzi operator or use Redpanda Cloud
   - Create required topics: `task-events`, `reminders`, `task-updates`
   - Configure Dapr Kafka pub/sub component

4. **Deploy with Helm**:
   - Update Helm charts to include Dapr annotations and components
   - Deploy application with Dapr sidecars to OKE
   - Configure Dapr components for pub/sub, state, bindings, and secrets

5. **Verify Deployment**:
   - Check pods with Dapr sidecars: `kubectl get pods -l dapr.io/enabled=true`
   - Verify Kafka connectivity and topic creation
   - Test event-driven functionality and service communication
...
### Phase 5 DevOps Components
- **Event Stream Architect Agent**: Designs event-driven architecture and schemas
- **Kafka Operator Agent**: Manages Kafka clusters and topics
- **Dapr PubSub Integrator Agent**: Configures Dapr integration and components
- **DOKS Cloud Orchestrator Agent**: Orchestrates cloud deployment and networking
- **AIOps Cloud Governor Agent**: Monitors and governs cloud operations
