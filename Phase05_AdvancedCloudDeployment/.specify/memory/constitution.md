...
<!-- Modified Principles: Event-Driven First (NEW), Dapr Abstraction Layer (NEW), Cloud-Native Portability (NEW) -->
<!-- Added Sections: Phase 5: Advanced Cloud Deployment, Event-Driven Architecture, Dapr Integration, Kafka Topics, Microservices Architecture -->
<!-- Removed Sections: None (All previous phases retained) -->
...
<!-- Ratification Date: 2026-01-02 (Phase 2), Last Amended: 2026-02-07 (Phase 5 Event-Driven Architecture) -->

# Hackathon II: Todo Full-Stack Web Application Constitution

**Project**: Taskify - Multi-user Full-Stack Todo Web Application with AI Chatbot and Advanced Cloud Deployment
**Phases**: Phase 2-5 Complete (Full-Stack Web App + AI Chatbot + Local Kubernetes + Advanced Cloud Deployment)
**Development Methodology**: Strict Spec-Driven Development (SDD) with Claude Code + Spec-Kit Plus + AI-Assisted DevOps + Event-Driven Architecture
...
- [Phase 5: Advanced Cloud Deployment](#phase-5-advanced-cloud-deployment)
...
- **Backend**: 80%+ overall coverage, 100% for authentication, security, and event publishing
- **Frontend**: 70%+ overall coverage, 90%+ for critical components
- **E2E**: 100% coverage for critical user flows
- **Security Tests**: MANDATORY (user isolation, SQL injection, XSS, event-level security)
- **Kubernetes Tests**: 100% coverage for deployment, scaling, and health checks
- **Event Flow Tests** (Phase 5): 100% coverage for event publishing, consumption, and processing
- **Dapr Integration Tests** (Phase 5): Verify pub/sub, state management, and service invocation
...
- **Event Stream Architect Agent** (Phase 5): Design event-driven architecture and schemas (`subagent_type=event-stream-architect`)
- **Kafka Operator Agent** (Phase 5): Manage Kafka clusters and topics (`subagent_type=kafka-operator`)
- **Dapr PubSub Integrator Agent** (Phase 5): Configure Dapr integration with Kafka (`subagent_type=dapr-pubsub-integrator`)
- **DOKS Cloud Orchestrator Agent** (Phase 5): Orchestrate cloud deployment pipelines (`subagent_type=doks-cloud-orchestrator`)
- **AIOps Cloud Governor Agent** (Phase 5): Monitor and govern cloud operations (`subagent_type=aiops-cloud-governor`)
...
- **Technologies Requiring Lookup**:
  ...
  - **Dapr (Distributed Application Runtime)** (Phase 5)
  - **Apache Kafka / Redpanda** (Phase 5)
  - **Strimzi Operator** (Phase 5)
  - **Event-Driven Architecture Patterns** (Phase 5)
...
Build with clean architecture for easy evolution across all phases:

- **Phase II Ready**: Full-stack web application (✅ Completed)
- **Phase III Ready**: AI chatbot integration with MCP tools (✅ Completed)
- **Phase IV Ready**: Kubernetes deployment with containerization (✅ Completed)
- **Phase V Ready**: Event-driven architecture with Kafka + Dapr (🚧 Current Phase)
- **Clean Separation**: Frontend ↔ API ↔ Business Logic ↔ Data Layer ↔ MCP Server ↔ Kubernetes ↔ Event Bus
- **Versioned APIs**: API versioning strategy for backward compatibility
- **Event Versioning**: Event schema versioning for backward-compatible event evolution
...
Phase 5 introduces event-driven architecture with Kafka and Dapr:

- **Event-Based Communication**: All business events (task created, updated, completed, deleted) MUST be published to Kafka topics
- **No Direct HTTP Between Services**: Microservices MUST NOT make direct HTTP calls to each other for business events
- **Synchronous Only for Users**: Direct HTTP calls allowed ONLY for user-facing request/response patterns (frontend → backend API)
- **Decoupled Services**: Producer services publish events without knowledge of consumers; consumer services subscribe without knowledge of producers
- **Event Schema Validation**: All events MUST conform to defined schemas with versioning support

**Rationale**: Event-driven architecture enables loose coupling, scalability, resilience, and independent service evolution—critical for production cloud deployments.

### XIII. Dapr Abstraction Layer (Phase 5 - NON-NEGOTIABLE)

Applications MUST use Dapr sidecars for all infrastructure interactions:

- **No Direct Kafka Libraries**: Applications MUST NOT have direct dependencies on kafka-python, aiokafka, or other Kafka client libraries
- **Dapr HTTP API**: All pub/sub operations via Dapr sidecar HTTP API (localhost:3500)
- **Infrastructure Swappable**: Kafka can be replaced with RabbitMQ, Azure Service Bus by changing Dapr component YAML—NO code changes
- **State Management**: Use Dapr state stores for conversation state where appropriate
- **Secrets Management**: Access secrets via Dapr secrets API or Kubernetes secrets

**Rationale**: Dapr abstracts infrastructure dependencies, enabling cloud portability, vendor flexibility, and cleaner application code.

### XIV. Cloud-Native Portability (Phase 5 - NON-NEGOTIABLE)

Deployment MUST work across multiple Kubernetes platforms:

- **Multi-Cloud Support**: Deployment configurations MUST work on Minikube (local), Oracle OKE, Azure AKS, and Google GKE
- **Helm Values Strategy**: Use environment-specific values files (values-minikube.yaml, values-staging.yaml, values-production.yaml)
- **No Cloud Provider Lock-In**: Avoid cloud-specific services or APIs; use Kubernetes-native patterns and Dapr abstractions
- **Environment Parity**: Maintain consistency across environments (local dev mirrors production architecture)

**Rationale**: Multi-cloud portability prevents vendor lock-in, enables cost optimization, and provides disaster recovery options.
...
Phase 5 extends Phase 4 with advanced cloud deployment using event-driven architecture, Dapr, and managed cloud services. The application is transformed into a distributed, event-driven system deployed to Oracle Cloud (OKE) with Kafka message broker.

### Part A: Advanced Features

**Event-Driven Architecture Implementation:**
- Kafka message broker for asynchronous communication
- Dapr for pub/sub abstraction and service mesh
- Event sourcing patterns for audit trails
- CQRS (Command Query Responsibility Segregation) patterns where appropriate

**Already Implemented Features (Phase 2):**
- ✅ Recurring Tasks
- ✅ Due Dates & Reminders
- ✅ Priorities
- ✅ Tags
...
- Deploy application to Minikube with full event-driven architecture
- Deploy Dapr on Minikube with full capabilities:
  - Pub/Sub (Kafka integration)
  - State Management (PostgreSQL backend)
...
- Deploy Kafka using Strimzi operator on Minikube
...
**Target Platform**: Oracle Cloud Infrastructure (OKE - Oracle Kubernetes Engine)
- **Free Tier**: 4 OCPUs, 24 GB RAM, Always Free tier
- **No Credit Card Charges**: After trial period
- **Best for Learning**: No time pressure, permanent free resources

**Deployment Requirements:**
- Deploy to Oracle OKE with production-grade configuration
- Deploy Dapr on OKE with full capabilities
- Use Kafka on Redpanda Cloud Serverless (free tier) or self-hosted with Strimzi
...
| Bindings (Jobs API) | Scheduled reminders at exact time |
...
#### Publishing Events (No Kafka Library)

**Without Dapr (PROHIBITED):**
```python
...
await httpx.post(
    ...
    json={"event_type": "created", "task_id": 1, "user_id": "xyz"}
)
```
...
async def handle_task_event(request: Request):
    ...
    # Process event (e.g., update audit log, trigger recurring task creation)
...
| Service | Purpose | Deployment | Communication |
|---------|---------|------------|---------------|
| **Frontend** | UI only, no business logic | Kubernetes Deployment | REST API to Backend |
| **Backend (Chat API)** | Chat + Task CRUD + Event publishing | Kubernetes Deployment | Publishes to Kafka via Dapr |
| **Notification Service** | Consume reminders, send notifications | Kubernetes Deployment | Consumes from `reminders` topic |
| **Recurring Task Service** | Consume task.completed, create next occurrence | Kubernetes Deployment | Consumes from `task-events` topic |
| **Audit Service** | Consume all events, maintain history | Kubernetes Deployment (optional) | Consumes from `task-events` topic |

### Dapr Sidecar Injection

All services MUST have Dapr sidecars injected via Kubernetes annotations:
...
- Frontend → Backend: Direct HTTP/REST (user requests)

**Asynchronous (Event-Driven):**
- Backend → Kafka → Notification Service (reminders)
- Backend → Kafka → Recurring Task Service (task completion)
- Backend → Kafka → Audit Service (all events)
...
### Phase 5 Event-Driven Stack

- **Messaging**: Apache Kafka 3.x (via Redpanda Cloud or Strimzi operator)
- **Distributed Runtime**: Dapr 1.12+
- **Message Format**: JSON with schema validation
- **Event Store**: Kafka topics with retention policies
- **Cloud Platform**: Oracle Cloud Infrastructure (OKE - Oracle Kubernetes Engine)
- **Free Tier**: 4 OCPUs, 24 GB RAM, Always Free
- **Microservices**: Backend (Chat API), Notification Service, Recurring Task Service, Audit Service (optional)
...
- **Phase 5 Agents**: event-stream-architect, kafka-operator, dapr-pubsub-integrator, doks-cloud-orchestrator, aiops-cloud-governor
...
- **Database**: Neon (serverless PostgreSQL, shared across all phases)
- **Phase 4 Target**: Minikube (local Kubernetes cluster)
- **Phase 5 Target**: Minikube (local with Kafka + Dapr) → Oracle OKE (cloud production)
...
### Prohibited Actions
- ❌ Manual code writing or editing (incl. Docker/YAML/CLI/Dapr components/Kafka configs)
...
- ❌ **Direct Kafka client libraries (kafka-python, aiokafka) - use Dapr** (Phase 5)
- ❌ **Direct HTTP calls between microservices for business events - use Kafka/Dapr** (Phase 5)
- ❌ **Blocking operations in event consumers** (Phase 5)

### Required Actions
- ✅ Use Context7 MCP for tech documentation (including Dapr and Kafka)
...
- ✅ **Use Dapr for ALL infrastructure interactions** (Phase 5)
- ✅ **Publish events for ALL task operations** (Phase 5)
- ✅ **Implement idempotent event consumers** (Phase 5)
- ✅ **Deploy to Minikube first, then Oracle OKE** (Phase 5)
- ✅ **Use specialized Phase 5 agents** (event-stream-architect, kafka-operator, dapr-pubsub-integrator)
...
- ✅ Dapr abstraction layer used throughout
- ✅ No direct Kafka dependencies in application code
- ✅ All task operations publish events to Kafka
- ✅ Notification service consumes reminders
- ✅ Recurring task service consumes completions
- ✅ Event consumers are idempotent
- ✅ Event schemas validated and versioned
- ✅ Deployed to Minikube with Kafka + Dapr
- ✅ Deployed to Oracle OKE (cloud production)
- ✅ CI/CD pipeline operational (GitHub Actions)
- ✅ Monitoring and observability implemented
- ✅ Multi-environment Helm values working
- ✅ Zero manual code (all AI-generated via Claude Code)
- ✅ Pure spec-driven implementation
- ✅ Specialized Phase 5 agents used (event-stream-architect, kafka-operator, dapr-pubsub-integrator)
...
**Amendment Version**: 1.3.0 → 2.0.0

**Rationale**: Addition of Phase 5 requirements for Advanced Cloud Deployment with Event-Driven Architecture, Dapr abstraction layer, and Kafka message broker. This represents a MAJOR version change because it introduces backward-incompatible architectural principles: event-driven first pattern, prohibition of direct Kafka dependencies, and mandatory Dapr usage for infrastructure interactions.

**Modified/Added Principles:**
- ✅ XII. Event-Driven First (Phase 5): NEW - All business events MUST go through Kafka/Dapr
- ✅ XIII. Dapr Abstraction Layer (Phase 5): NEW - No direct Kafka libraries allowed
- ✅ XIV. Cloud-Native Portability (Phase 5): NEW - Multi-cloud support (Minikube, OKE, AKS, GKE)
- ✅ IV. Use Subagents and Skills: Added Phase 5 specialized agents (event-stream-architect, kafka-operator, dapr-pubsub-integrator, doks-cloud-orchestrator, aiops-cloud-governor)
- ✅ V. Context7 MCP Server: Added Phase 5 technologies (Dapr, Kafka, Redpanda, Strimzi, Event-Driven Architecture)
- ✅ III. Test-Driven Development: Added Phase 5 testing requirements (event flow tests, Dapr integration tests)
- ✅ VIII. Modular & Future-Ready Design: Updated with Phase 5 event bus integration
- ✅ Updated Constraints: Added Phase 5 prohibitions and requirements
- ✅ Updated Success Criteria: Added Phase 5 requirements

**New Major Sections:**
- ✅ Phase 5: Advanced Cloud Deployment: Complete section on event-driven architecture and cloud deployment
- ✅ Event-Driven Architecture: Kafka use cases, topics, event schemas
- ✅ Dapr Integration: Dapr building blocks, component definitions, usage patterns
- ✅ Microservices Architecture: Service boundaries, Dapr sidecar injection, communication patterns

**Removed Sections:**
- None (All previous phases retained for backward compatibility)

**Templates Requiring Updates:**
- ✅ `@specs/templates/spec-template.md` - Add Phase 5 event-driven specs section
- ✅ `@specs/templates/plan-template.md` - Add Kafka/Dapr architecture planning
- ✅ `@specs/templates/tasks-template.md` - Add event publishing/consuming task templates

**Follow-up Tasks:**
- Create Phase 5 specification files in `specs/phase-5/`
- Create event-driven architecture specs in `specs/phase-5/event-driven-architecture.md`
- Create Kafka specs in `specs/phase-5/kafka-integration.md`
- Create Dapr specs in `specs/phase-5/dapr-integration.md`
- Create microservices specs in `specs/phase-5/microservices.md`
- Create notification service specs in `specs/phase-5/notification-service.md`
- Create recurring task service specs in `specs/phase-5/recurring-task-service.md`
- Update CLAUDE.md files to reference Phase 5 workflows
- Create event flow testing specifications
- Create Dapr integration testing specifications
- Update Helm charts for Dapr sidecar injection
- Create Kafka deployment specifications (Strimzi operator)
- Set up Dapr components in `/dapr-components/`
...
**Amendment Type**: MAJOR (Event-driven architecture principles, Dapr abstraction layer, removed direct Kafka dependencies)
**Next Review**: After Phase 5 completion for production lessons learned
**Status**: Active for Phase 5 (All previous phases still valid as foundation)