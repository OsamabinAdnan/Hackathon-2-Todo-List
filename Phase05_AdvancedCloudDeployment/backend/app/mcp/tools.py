...
Phase 5: Enhanced with event publishing via Dapr pub/sub.
"""
...
# Phase 5: Event publishing imports
...
def add_task(
    ...
    try:
        ...
        # Phase 5: Publish task.created event (FR-001) - non-blocking
        try:
            asyncio.create_task(publish_task_created(task, user_uuid))
        except Exception as e:
            logger.warning(f"Failed to publish task.created event from MCP tool: {e}")
...
def complete_task(
    ...
    try:
        ...
        # Phase 5: Publish task.completed or task.updated event (FR-003) - non-blocking
        if completed:
            try:
                asyncio.create_task(publish_task_completed(task, user_uuid))
            except Exception as e:
                logger.warning(f"Failed to publish task.completed event from MCP tool: {e}")
        else:
            # Task was re-opened, publish task.updated event
            try:
                asyncio.create_task(publish_task_updated(task, user_uuid))
            except Exception as e:
                logger.warning(f"Failed to publish task.updated event from MCP tool: {e}")
...
def update_task(
    ...
    try:
        ...
        # Phase 5: Publish task.updated event (FR-002) - non-blocking
        try:
            asyncio.create_task(publish_task_updated(task, user_uuid, previous_task))
        except Exception as e:
            logger.warning(f"Failed to publish task.updated event from MCP tool: {e}")
...
def delete_task(
    ...
    try:
        ...
        if len(tasks) == 0:
            ...
            # Phase 5: Publish task.deleted event (FR-004) - non-blocking
            try:
                asyncio.create_task(publish_task_deleted(deleted_task_id, user_uuid))
            except Exception as e:
                logger.warning(f"Failed to publish task.deleted event from MCP tool: {e}")

            return {
                "success": True,
                "message": f"Task '{deleted_task_title}' deleted."
            }
    except Exception as e:
