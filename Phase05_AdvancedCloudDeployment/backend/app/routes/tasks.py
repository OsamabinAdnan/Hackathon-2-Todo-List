...
from app.publishers.task_event_publisher import (
    publish_task_created,
    publish_task_updated,
    publish_task_completed,
    publish_task_deleted
)
# Phase 5: Reminder scheduler for Dapr Jobs API
...
async def create_task(
    ...
    **Status Codes:**
    ...
    # Publish task.created event (FR-001) - non-blocking
    try:
        asyncio.create_task(publish_task_created(db_task, uuid.UUID(user_id)))
    except Exception as e:
        logger.warning(f"Failed to publish task.created event: {e}")

    # Phase 5: Schedule reminder if task has due_date (T047, FR-008)
    if db_task.due_date:
        try:
            scheduler = get_reminder_scheduler()
            asyncio.create_task(scheduler.schedule_reminder(
                task_id=db_task.id,
                user_id=uuid.UUID(user_id),
                title=db_task.title,
                due_date=db_task.due_date,
                remind_before_minutes=30  # Default: remind 30 minutes before due
            ))
        except Exception as e:
            logger.warning(f"Failed to schedule reminder for task {db_task.id}: {e}")
...
async def update_task(
    ...
    **Status Codes:**
    ...
    # Capture previous state for event (FR-002)
    previous_task = Task(
        id=db_task.id,
        title=db_task.title,
    ...
    # Publish task.updated event (FR-002) - non-blocking
    try:
        asyncio.create_task(publish_task_updated(db_task, uuid.UUID(user_id), previous_task))
    except Exception as e:
        logger.warning(f"Failed to publish task.updated event: {e}")

    # Phase 5: Reschedule reminder if due_date changed (T048, FR-010)
    if 'due_date' in update_data:
        try:
            scheduler = get_reminder_scheduler()
            if db_task.due_date:
                # Reschedule reminder with new due date
                asyncio.create_task(scheduler.reschedule_reminder(
                    task_id=db_task.id,
                    user_id=uuid.UUID(user_id),
                    title=db_task.title,
                ...
                asyncio.create_task(scheduler.cancel_reminder(db_task.id))
        except Exception as e:
            logger.warning(f"Failed to reschedule reminder for task {db_task.id}: {e}")
...
async def delete_task(
    ...
    **Warning:**
    ...
    # Store task_id before deletion for event publishing
    deleted_task_id = db_task.id

    # Phase 5: Cancel any scheduled reminder (T049, FR-011)
    try:
        scheduler = get_reminder_scheduler()
        asyncio.create_task(scheduler.cancel_reminder(deleted_task_id))
    except Exception as e:
        logger.warning(f"Failed to cancel reminder for task {deleted_task_id}: {e}")

    session.delete(db_task)
    session.commit()

    # Publish task.deleted event (FR-004) - non-blocking
    try:
        asyncio.create_task(publish_task_deleted(deleted_task_id, uuid.UUID(user_id)))
    except Exception as e:
...
async def toggle_task_completion(
    ...
    **Status Codes:**
    ...
    # Publish task.completed event (FR-003) if task was completed - non-blocking
    if completion_data.completed:
        try:
            asyncio.create_task(publish_task_completed(db_task, uuid.UUID(user_id)))
        except Exception as e:
            logger.warning(f"Failed to publish task.completed event: {e}")
    else:
        # Task was re-opened, publish task.updated event
        try:
            asyncio.create_task(publish_task_updated(db_task, uuid.UUID(user_id)))
        except Exception as e:
            logger.warning(f"Failed to publish task.updated event: {e}")
