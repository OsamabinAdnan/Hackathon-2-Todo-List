...
Phase 5: Enhanced with optional Dapr state caching (database remains source of truth)
"""
...
# Phase 5: Optional Dapr state caching
from app.services.dapr_state import get_state_service
...
# Phase 5: Helper function for optional conversation state caching
async def _cache_conversation_state(
    conversation_id: uuid.UUID,
    user_id: str,
    message_history: List[Dict[str, str]]
) -> None:
    """
    Optionally cache conversation state to Dapr state store.
    This is a performance optimization - database remains source of truth.
    ...
    try:
        ...
        await state_service.save(cache_key, cache_data)
    except Exception as e:
        logger.debug(f"Optional cache update failed (non-critical): {e}")
...
async def chat(
    ...
    # Phase 5: Optional cache update (non-blocking, database is source of truth)
    try:
        asyncio.create_task(_cache_conversation_state(
            conversation_id,
            user_id,
            message_history
        ))
