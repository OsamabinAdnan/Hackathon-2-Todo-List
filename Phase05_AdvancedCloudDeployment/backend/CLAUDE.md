...
## Phase 5: Backend for Advanced Cloud Deployment
...
### Phase 5 Backend Technology Stack
- **Event Streaming**: Apache Kafka (self-hosted with Strimzi or managed with Redpanda Cloud)
- **Distributed Runtime**: Dapr (Distributed Application Runtime) for pub/sub, state management, and service invocation
- **Pub/Sub Integration**: Dapr with Kafka as underlying transport for event-driven architecture
- **State Management**: Dapr state stores for conversation and message persistence
- **Service Invocation**: Dapr for inter-service communication with built-in retries
- **Bindings**: Dapr cron bindings for scheduled tasks and reminders
- **Secrets Management**: Dapr secret stores with Kubernetes secrets backend
- **Cloud Platform**: Oracle Cloud Infrastructure (OKE) - Always Free tier
- **AI-Assisted Operations**: kubectl-ai, kagent, and specialized agents for cloud operations

### Phase 5 Backend Architecture
- **Event-Driven Architecture**: All services communicate through Kafka topics via Dapr pub/sub
- **Dapr Integration**: Dapr sidecars handle messaging, state, and service invocation
...
- **Scalable Design**: Event-driven architecture enables horizontal scaling
- **Resilient Systems**: Async processing with dead letter queues and retry policies
- **Observability**: Distributed tracing and monitoring across services and events

### Phase 5 Backend Components
...
- **Pub/Sub**: Kafka abstraction via Dapr (no Kafka client code needed)
- **State Management**: Conversation and message state storage using Dapr state stores
- **Service Invocation**: Inter-service communication with built-in retries
- **Bindings**: Cron triggers for scheduled reminders and recurring tasks
...
- **Recurring Task Service**: Consumes task completion events and creates next occurrences
- **Audit Service**: Consumes task events and maintains complete history
- **WebSocket Service**: Consumes task update events and syncs clients in real-time

### Phase 5 Backend Integration Points
- **MCP Server Integration**: MCP tools publish events via Dapr pub/sub
- **Task Operations**: Create, update, delete operations publish to `task-events` topic
- **Reminder System**: Due date events trigger reminders published to `reminders` topic
- **Real-time Sync**: Task updates published to `task-updates` for client synchronization
- **Authentication**: Continue using JWT-based authentication with enhanced security
- **User Isolation**: Maintain same security model with user-specific event filtering
- **Database**: Continue using Neon PostgreSQL with Dapr state management for conversations

### Phase 5 Backend Deployment Architecture
- **Backend Service**: FastAPI + MCP tools deployed as Kubernetes Deployment with Dapr sidecar
- **Kafka Cluster**: Self-hosted with Strimzi operator or managed with Redpanda Cloud
- **Dapr Sidecar**: Handles pub/sub, state management, and service invocation
- **Dapr Components**: Kafka pub/sub, PostgreSQL state store, Kubernetes secrets
- **Oracle Cloud OKE**: Production-grade Kubernetes cluster with free tier benefits
- **Load Balancers**: Cloud-native load balancing with SSL termination
- **Monitoring**: Cloud-native monitoring and logging solutions

### Phase 5 Backend Development Requirements
1. **Dapr Client Integration**:
   - Implement Dapr pub/sub client for publishing task events
   - Use Dapr state management for conversation/message persistence
   - Leverage Dapr service invocation for inter-service communication

2. **Event Publishing**:
   - Publish `task-events` when tasks are created/updated/completed/deleted
   - Publish `reminders` when due dates are set or approaching
   - Publish `task-updates` for real-time client synchronization

3. **State Management**:
   - Store conversation state using Dapr state stores
   - Implement message persistence with user isolation
   - Handle state conflicts and concurrency

4. **MCP Tool Enhancement**:
   - Update MCP tools to publish events via Dapr pub/sub
   - Add event correlation IDs for distributed tracing
   - Implement event-based task operations

5. **Error Handling**:
   - Implement circuit breakers for service invocation
   - Handle pub/sub connection failures gracefully
   - Provide dead letter queue mechanisms for failed events

### Phase 5 Backend Testing Requirements
- **Event Flow Tests**: 100% coverage for event publishing, consumption, and processing
- **Dapr Integration Tests**: Verify pub/sub, state management, and service invocation
- **End-to-End Event Tests**: Test complete event-driven workflows (task → event → consumer)
- **Scalability Tests**: Validate horizontal scaling with increased event load
- **Fault Tolerance Tests**: Test system resilience with failing consumers and network partitions
- **Security Tests**: Verify Dapr security, event encryption, and access controls
- **Performance Tests**: Validate event processing latency and throughput
- **MCP Tool Tests**: Validate MCP tools with event-driven architecture
